...
求人広告に添える短い動画を、もっと手軽に作れないか——という相談がきっかけだった。
...
動画はサーバーに一切送信されない。処理はすべてユーザーのブラウザ内で完結するため、Vercelの処理時間制限（Hobbyプランで10秒）にも引っかからない。機密性の高い採用関連の動画を扱う上でも、この構成は都合が良かった。
...
完璧な自動化からはほど遠い。でも、「動画を選んで、ボタンを押して、少し直して、ダウンロードする」——その4ステップで求人動画が仕上がる仕組みが、ゼロ円で動いている。今のところは、それで十分だと思っている。
